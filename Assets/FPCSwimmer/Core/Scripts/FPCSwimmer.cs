    ...
    public class FPCSwimmer : MonoBehaviour
    {
        ...
        private bool CalculateNewPosition(bool isSwimming)
        {
            ...
            // Apply rotation according to the player's input, regardless of location movements.
            {
                ...
                //rotX = camara.transform.rotation.y;
                //rotY = camara.transform.rotation.x;
                //float rotZ = camara.transform.rotation.z;

                // -Hacer que se mueva en direccion a la camara-
                transform.localRotation = Quaternion.Euler(camara.transform.forward);
            }
            ...
            // Start collecting information on the resulting motion for the player
            {
                ...
                // -Mover siempre en la direccion de la cámara siempre que esté nadando-
                if (Math.Abs(Input.GetAxis("Vertical")) > 0.1 && Math.Abs(Input.GetAxis("Horizontal")) < 0.1)
                {
                    if (Input.GetAxis("Vertical") > 0)
                    {
                        playerVelocityVector = camara.transform.forward;
                    }
                ...
                // Follow mouse (camara) direction as a moving direction
            ...
            else // -Nunca debería entrar aquí-
            {
            ...
            cc.Move(playerVelocityVector * Time.deltaTime * playerSpeed * 3);
