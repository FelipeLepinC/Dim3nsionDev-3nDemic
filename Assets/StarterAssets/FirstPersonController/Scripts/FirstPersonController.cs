	...
	public class FirstPersonController : MonoBehaviour
	{
		...
		private void Update()
		{
			...
			if (Input.GetKeyDown(KeyCode.R)){
			   manzanas = ui_contador.GetComponent<AppleCounter>().counter;
			   Debug.Log(manzanas);
			   if(manzanas >= 10 && boost == false){
					ui_contador.GetComponent<AppleCounter>().Boost();
			   		StartCoroutine(Boost());
			   }else{
				Debug.Log("Falta comida para activar el boost");
			   }
		...
		IEnumerator Boost(){
			...
			SpeedChangeRate = 2 * SpeedChangeRate;
			yield return new WaitForSeconds(3);
			MoveSpeed = MoveSpeed / 2;
			SprintSpeed = SprintSpeed / 2;
			RotationSpeed = RotationSpeed / 2;
			SpeedChangeRate = SpeedChangeRate / 2;
			boost = false;
		}
	}
}